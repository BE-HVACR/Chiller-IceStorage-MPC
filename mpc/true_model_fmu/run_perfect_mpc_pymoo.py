...
Using Pymoo package for binary optimization problem
'''
...
class PerfectMPC(object):
    ...
    #def get_fmu_outputs(self, fmu_results):
        ...
        for output_name in self.fmu_output_names:
            ...
            #outputs_dict[output_name] = fmu_results[output_name]

        time = self._fmu_results['time']
        #time = fmu_results['time']
    ...
    def objective(self, u):
        ...
        for i, name in enumerate(self.fmu_input_names):
            self.fmu_model.set(name,u[i])
       
        outputs = self.fmu_model.simulate(start_time=ts, final_time=te, options=self.fmu_options)
        outputs = self.get_fmu_outputs(outputs)
        '''
        ...
        #outputs = self._sample(self._interpolate(outputs, t_intp), 60-1)
    ...
    def _calculate_temperature_violation(self, outputs):
        ...
        T_lower[self.occ_start:self.occ_end] = [23.]*(self.occ_end-self.occ_start)
        ...
        for zone_temp in zone_temp_list:
            ...
            #temp.append(outputs[zone_temp] - 273.15)
            temp.append(temp_z.tolist())
        print("\ntemp:",temp)

        #temp = outputs['mod.flo.temAirPer5.T'] - 273.15  
    ...
    def _calculate_action_changes(self, u):
        ...
        return du
    '''
    def __calculate_action_changes(self,outputs):
        uncon_u = self.fmu_uncontrollable_names
        u_pre = []    #prepare u list
        for t in range(PH):
            for un in uncon_u:
                _u_uncon_i = outputs[un].tolist()
                u_uncon_i = _u_uncon_i[t]
                u_pre.append(u_uncon_i)
        return u_pre
    
    def _calculate_action_changes(self, u, outputs):
        # # of inputs
        ni = self.ni
        ni_un = 10 - ni
        # get bounds
        u_lb = [0, 0, 5+273.15, 15.6+273.15, 11.8+273.15, 0.23, 0.05, 0.05, 0.05, 0.04] 
        u_ub = [1, 1, 10+273.15, 29.4+273.15, 18+273.15, 4.5, 0.90, 0.95, 0.95, 0.70]        
        order = self.control_name_order
        u_ch_prev = u_lb
        
        u_pre = self.__calculate_action_changes(outputs)
        du_nomalizer = [1./(u_ub[i] - u_lb[i]) for i in range(len(u_lb))]*self.PH
        du = []
        for i in range(self.PH):
            u_con = u[i*ni:ni*(i+1)]       #controled u
            ui = u_pre[i*ni_un:ni_un*(i+1)]   #define all controllable variable list and add uncontrolled u
            for k in range(ni):
                ui.insert(order,u_con[k])      #add controlled u_con[0]
            dui_nomalizer = du_nomalizer[i*(ni+ni_un):(ni+ni_un)*(i+1)]
            dui = [abs(ui[j] - u_ch_prev[j])*dui_nomalizer[j] for j in range(ni)] 
            u_ch_prev = ui
            du += dui
        return du
...
if __name__ == "__main__":
    ...
    #model = load_fmu(os.path.join(LIBRARYPATH,"wrapped_fixed_curve1407_COP35_supFanEff70_pumEff70.fmu"))
    model = load_fmu('VirtualTestbed_NISTChillerTestbed_DemandFlexibilityInvestigation_FakeSystem_SystemForMPC_2bInputs_RealtoBool.fmu', log_level=3) # Enable logging for more information
    ...
    #input_control=[Tchw, Tcw, Tsa, Vcore, Veast, Vnorth, Vsouth, Vwest, \epsilon] 
    control_names = []
    active_names = []
    for name in inputs:
    ...
    '''        
    control_names = ['oveTChiWatSupSet_u']
    active_names = ['oveTChiWatSupSet_activate']
    
    uncontrollable_names = ['mod.conAHU.ySupFan',
                            'mod.chiPlaEnaDis.yPla',
    ...
    mpc = PerfectMPC(PH = PH,
                    ...
                    fmu_model = model, 
                    fmu_generator="dymola",
                    #control_name_order = control_name_order,
    ...
    results.to_csv('results_opt_PH'+str(PH)+'.csv')
