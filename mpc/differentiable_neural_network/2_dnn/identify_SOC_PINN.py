...
# Save the figure
plt.savefig('./results_pinn/SOC_scatter.png')
plt.show()
...
plt.legend(loc='upper right')
plt.title('Trained NN model with RMSE = {:.2f} and CVRMSE = {:.2f}%'.format(RMSE_test, RMSE_test/mean_test*100), fontsize=14)
...
plt.savefig('./results_pinn/SOC_error.png')
plt.show()
...
nn_model = tf.keras.models.load_model('./results_pinn/pinn_SOC_model.h5',
...
plt.savefig(f'./results_pinn/open_loop_test_SOC.png')
plt.tight_layout()
plt.show()
...
plot_sensitivity_analysis(
                            ...
                            save_path = './results_pinn/sensitivity_analysis_SOC.png',
                        )
