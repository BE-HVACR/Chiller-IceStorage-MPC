...
from keras_to_casadi import convert_keras_to_casadi, PowerOutputMasked
import random
import os
...
class mpc_case():
    def __init__(self,PH,CH,time,dt,measurement,states,predictor):
        ...
        self.w = [1., 1., 1.]  # weights in objective function: : Minimize energy cost + temperature violation + SOC penalty; SOC is hard constraint in this case, so no weight is assigned to it
    ...
    def optimize(self):
        ...
        for k in range(self.PH):
            ...
            P_pred_ph[k] = power_dnn(ca.vertcat(u[0], To_pred_ph[k], Tz_ave_his_meas_k[0])) #* ca.fabs(u[0])
    ...
    def get_open_loop_preds(self, u_opt_ph):
        ...
        for k in range(self.PH):
            ...
            #                                     GHI_his_meas_k[0], GHI_pred_ph[k],
            #                                     P_his_meas_k[0]))  ) * ca.fabs(u[0])
            P_pred_ph[k] = float(power_dnn(ca.vertcat(u[0], To_pred_ph[k], Tz_ave_his_meas_k[0])))#* ca.fabs(u[0]))
    ...
    def get_power_pred(self, u):
        ...
        P = power_dnn(ca.vertcat(u[0], self.predictor['Toa'][0], self.states['Tz_ave_his_meas'][0])) # self.states['Tz_ave_his_meas'][0], self.states['To_his_meas'][0], , u[1], occupied, self.states['SOC_his_meas'][0], 
    ...
    #     '''
    #     model_path="../../2_dnn/results_pinn/pinn_power_model.h5"
    ...
    def power_dnn_tensorflow(self):
        '''Differentiable Neural Network for power prediction using masked output'''

        model_path = "../../2_dnn/results_pinn/pinn_power_model.h5"

        # === Register custom layer and load model safely ===
        with tf.keras.utils.custom_object_scope({'PowerOutputMasked': PowerOutputMasked}):
            nn_model_casadi = convert_keras_to_casadi(model_path)

        # === Create symbolic input ===
        input_dim = nn_model_casadi.sx_in()[0].shape[0]
        nn_input = ca.MX.sym('nn_input', input_dim)

        # === Symbolic forward evaluation ===
        nn_output = nn_model_casadi(nn_input)

        # === Return CasADi Function ===
        return ca.Function('nn_model', [nn_input], [nn_output])
    ...
    def SOC_dnn_tensorflow(self):
        ...
        # === Step 1: Load and Convert the TensorFlow Model to CasADi ===
        model_path = "../../2_dnn/results_pinn/pinn_SOC_model.h5"
        nn_model_casadi = convert_keras_to_casadi(model_path)
        
        # === Step 2: Create Symbolic Input to Pass to CasADi Function ===
        input_shape = nn_model_casadi.sx_in()[0].shape[0]  # number of input variables
        nn_input = ca.MX.sym('nn_input', input_shape)
        
        # === Step 3: Evaluate the Network Output Symbolically ===
        nn_output = nn_model_casadi(nn_input)
        
        # === Step 4: Wrap in a CasADi Function and Return ===
        return ca.Function('nn_model', [nn_input], [nn_output])
