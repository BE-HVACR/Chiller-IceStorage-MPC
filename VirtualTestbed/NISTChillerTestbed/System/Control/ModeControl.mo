...
model ModeControl "Storage mode controller"
  ...
  Modelica.StateGraph.Transition OffToCha(
    enableTimer=true,
    waitTime=waiTim,
    condition=Occupied == false and SOC < 0.99)
                                             "Off to charging mode" annotation (
     Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=90,
        origin={-40,22})));
  Modelica.StateGraph.Transition ChaToOff(
    enableTimer=true,
    waitTime=waiTim,
    condition=SOC >= 0.99)  "Charging mode to Off mode" annotation (Placement(
        transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={12,22})));
  Modelica.StateGraph.Transition ChaToDisSto(
    enableTimer=true,
    waitTime=waiTim,
    condition=Occupied == true) "Charging mode to dischargingstorage mode"
    annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=90,
        origin={-40,-20})));
  Modelica.StateGraph.Transition OffToDis(
    ...
    "Off to discharging mode" annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=90,
        origin={-74,22})));
  ...
  Modelica.StateGraph.Transition DisStoToDisChi(
    enableTimer=true,
    waitTime=waiTim,
    condition=Occupied == true and SOC <= 0.01)
    "discharging storage to discharging chiller" annotation (Placement(
        transformation(
        extent={{10,-10},{-10,10}},
        rotation=90,
        origin={-40,-56})));
  Modelica.StateGraph.Transition DisChiToOff(
    enableTimer=true,
    waitTime=waiTim,
    condition=Occupied == false) "Dischargingchiller mode to off mode"
    annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={28,-56})));
  Modelica.StateGraph.Transition DisStoToOff(
    enableTimer=true,
    waitTime=waiTim,
    condition=Occupied == false) "Dischargingstorage mode to off mode"
    annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={18,-20})));
equation
  connect(Off.outPort[1], OffToCha.inPort) annotation (Line(points={{-10.3333,
          39.5},{-10.3333,34},{-40,34},{-40,26}},
                                          color={0,0,0}));
  connect(Off.outPort[2], OffToDis.inPort) annotation (Line(points={{-10,39.5},
          {-10,34},{-74,34},{-74,26}},  color={0,0,0}));
  ...
  connect(ChaToOff.outPort, Off.inPort[1]) annotation (Line(points={{12,23.5},{
          12,66},{-10.6667,66},{-10.6667,61}},
                                      color={0,0,0}));
  connect(OffToDis.outPort, DischargingStorage.inPort[2]) annotation (Line(
        points={{-74,20.5},{-74,-24},{-9.5,-24},{-9.5,-25}}, color={0,0,0}));
  ...
  connect(DisStoToDisChi.outPort, DischargingChiller.inPort[1]) annotation (
      Line(points={{-40,-57.5},{-40,-63},{-10.5,-63}}, color={0,0,0}));
  connect(DischargingChiller.outPort[1], DisChiToOff.inPort) annotation (Line(
        points={{-10.25,-84.5},{10,-84.5},{10,-84},{28,-84},{28,-60}}, color={0,
          0,0}));
  connect(DisChiToOff.outPort, Off.inPort[3]) annotation (Line(points={{28,
          -54.5},{28,80},{-9.33333,80},{-9.33333,61}}, color={0,0,0}));
  connect(DischargingStorage.outPort[1], DisStoToDisChi.inPort) annotation (
      Line(points={{-10.25,-46.5},{-10.25,-50},{-40,-50},{-40,-52}}, color={0,0,
          0}));
  connect(Charging.outPort[1], ChaToDisSto.inPort) annotation (Line(points={{
          -10.25,-10.5},{-10,-10.5},{-10,-16},{-40,-16}}, color={0,0,0}));
  connect(ChaToDisSto.outPort, DischargingStorage.inPort[1]) annotation (Line(
        points={{-40,-21.5},{-10,-21.5},{-10,-25},{-10.5,-25}}, color={0,0,0}));
